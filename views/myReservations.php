<?php
Session::isLogged();
$reservations =  ReservationControllers::getByUser();
?>
...
<div class="relative">
  <header class=" backdrop-blur-2xl fixed top-0 w-screen left-0" x-data="{ isOpen: false }">
  ...
  <form class="flex items-center mt-12 flex-col w-full">
    <div class="p-10 xl:w-3/4 sm:w-full">
      
      <?php foreach ($reservations as $flight) { ?>
        <div class="max-w-full my-3 bg-white flex flex-col rounded overflow-hidden shadow-lg" id="flightTickets">
          ...
          <div class="mt-2 flex sm:flex-row mx-6 sm:justify-between flex-wrap ">
            ...
            <!-- buttton Delete -->
            <button type="button" class="flex items-center px-5 py-2.5 font-medium tracking-wide text-black capitalize rounded-md  hover:bg-red-200 hover:fill-current hover:text-red-600  focus:outline-none  transition duration-300 transform active:scale-95 ease-in-out" name="delete"
              id="deleteButton"
            >
...
<script>
  ...
  const flightTickets = document.querySelectorAll('#flightTickets')
  const deleteButton = document.querySelectorAll('#deleteButton')

  for (let i = 0; i < flightTickets.length; i++) {
    deleteButton[i].onclick = () => {
      flightTickets[i].remove()
    }
